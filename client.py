...
class Client(Settings):
    ...
    def api_client(self):
        ...
        return conquest.ApiClient(configuration=config)
    ...
    def _test_host(self):
        ...
        print('testing host')
        try:
            output = conquest.SystemServiceApi(self.api_client).system_service_application_version_with_http_info()
            # print((output))
            print('ok')
            return True
    ...
    def list_children(self, uid: int, type: conquest.ConquestApiObjectType) -> set:
        # if not self.is_valid: return []
        body = conquest.ConquestApiListHierarchyNodesQuery(
            include_ancestors=False, 
            include_children=True, 
            include_sub_items=False,
            include_siblings=False,
            object_key=conquest.ConquestApiObjectKey(int32_value=uid, 
                                                    object_type=type)
        )
        output = conquest.ViewServiceApi(self.api_client).view_service_list_hierarchy_nodes(body=body)
        # depth = 0
        # filtered_output = []
    ...
    def create_asset(self, description: str, parent_id: int, type_id: int=None, wkt_geometry: str=None):
        ...
        body = conquest.ConquestApiCreateAssetCommand(
            asset_description=str(description), 
            parent_id=parent_id, 
            type_id=type_id,
            geography_data=geog
        )
        new_asset_id= conquest.AssetServiceApi(self.api_client).asset_service_create_asset(body=body)
